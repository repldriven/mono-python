...
# things are the workspace's own: the PRD checks run only where
# docs/prd/ exists, and the names the competitor check refuses are read
# from .config/check-docs/names, one per line.
...
# One readme whichever case it is spelled in: a case-insensitive disk
# answers to both names, and would report each finding twice.
TOP_MD=()
for f in readme.md README.md; do
  if [ -f "$f" ]; then TOP_MD+=( "$f" ); break; fi
done
[ -f CLAUDE.md ] && TOP_MD+=( CLAUDE.md )
...
# 7. Competitor names, from the workspace's own list.
section 'Competitor names'
NAMES_FILE=.config/check-docs/names
if [ -s "$NAMES_FILE" ]; then
  names=$(grep -v '^#' "$NAMES_FILE" | grep -v '^$' | paste -sd '|' -)
  out=$(grep -nwE "$names" "${PATTERN_MD[@]}" 2>/dev/null)
  report 'competitor-names' "$out"
else
  echo "PASS — competitor-names (no $NAMES_FILE)"
fi
